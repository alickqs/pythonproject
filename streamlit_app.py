...
image_path = os.path.join(os.path.dirname(__file__), "Gemini_Generated_Image_61g94l61g94l61g9.png")
st.image(image_path, use_container_width=True)

# DASHBOARD
st.header("Dashboard", anchor="dashboard")
st.title("Energy Consumption & Weather — Spain (2015–2018)")
st.markdown("Hourly electricity generation, load, and pricing data for Spain merged with meteorological observations from five major Spanish cities, covering 2015 through 2018.")
...
for i, season_name in enumerate(SEASON_ORDER):
    pivot = (df[df["season"] == season_name]
...
hourly = df.groupby("hour")[["price actual", "total load actual", "generation fossil gas", "generation wind onshore"]].mean().reset_index()
...
season_stats = df.groupby("season", observed=True).agg(
...
hourly_df = df.groupby("hour")[["total load actual", "price actual"]].mean().reset_index()
...
peak_load = df[df["is_peak_hour"] == 1]["total load actual"]
off_load = df[df["is_peak_hour"] == 0]["total load actual"]
t_l, p_l = stats.ttest_ind(peak_load, off_load, equal_var=False)

peak_price = df[df["is_peak_hour"] == 1]["price actual"]
off_price = df[df["is_peak_hour"] == 0]["price actual"]
